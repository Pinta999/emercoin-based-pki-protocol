...
#define EMC_CORE_URL "http://user:psw@192.168.1.18:9092"
...
static int my_verify( void *data, mbedtls_x509_crt *crt,
    ...
    if (strcmp(issuer_dn_buf, EMC_ISSUER_STR) != 0) {
        ...
        *flags |= MBEDTLS_X509_EMC_INCOMPATIBLE_ISSUER;
        mbedtls_printf("!    Wrong issuer\n");
        return MBEDTLS_X509_EMC_INCOMPATIBLE_ISSUER;
    ...
    if (curl) {
        ...
        if (res != CURLE_OK) {
            mbedtls_printf("\nError in curl request - error %d.\n", res);
            *flags |= MBEDTLS_X509_EMC_FAILED_TO_CONNECT_EMC_CORE;
            return MBEDTLS_X509_EMC_FAILED_TO_CONNECT_EMC_CORE;
        }
    ...
    r = jsmn_parse(&p, chunk.memory, strlen(chunk.memory), t, 128);

    if (r < 0) {
        mbedtls_printf( "Failed to parse JSON: %d\n", r);
        *flags |= MBEDTLS_X509_EMC_FAILED_TO_PARSE_JSON;
        return MBEDTLS_X509_EMC_FAILED_TO_PARSE_JSON;
    }

    if (r < 1 || t[0].type != JSMN_OBJECT) {
        mbedtls_printf("Object expected\n");
        *flags |= MBEDTLS_X509_EMC_TOP_LEVEL_ELEMENT_IS_NOT_OBJECT;
        return MBEDTLS_X509_EMC_TOP_LEVEL_ELEMENT_IS_NOT_OBJECT;
    }
    ...
    if (lookup_success < 0) {
        *flags |= MBEDTLS_X509_EMC_NO_NAME_IN_EMC_CORE;
        return MBEDTLS_X509_EMC_NO_NAME_IN_EMC_CORE;
    }
    ...
    unsigned char rawcert[1024] = {0};
    memcpy(rawcert, crt->tbs.p, crt->tbs.len);

    if (!&ctx) {
        mbedtls_printf("Unknown digest sha256\n");
        *flags |= MBEDTLS_X509_EMC_INVALID_HASH_FUNC;
        return MBEDTLS_X509_EMC_INVALID_HASH_FUNC;
    }

    mbedtls_md_starts(&ctx);

    if (mbedtls_md_update(&ctx, rawcert, strlen(rawcert)) != 0) {
        /* TODO: Error management */
        *flags |= MBEDTLS_X509_EMC_ERR_DIGEST_COMPUTATION;
        return MBEDTLS_X509_EMC_ERR_DIGEST_COMPUTATION;
    }
    ...
    for (i = 0; i < 64; i++) {
        if (md_buf_hex[i] != lookup_sha256_result[i]) {
            ...
            *flags |= MBEDTLS_X509_EMC_DIGEST_NOT_MATCHING;
            return MBEDTLS_X509_EMC_DIGEST_NOT_MATCHING;
        }
    }
