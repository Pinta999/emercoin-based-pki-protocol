...
const char * mbedtls_low_level_strerr( int error_code )
{
    ...
    switch (error_code) {
        case MBEDTLS_X509_EMC_ERR_DIGEST_COMPUTATION:
            return( "X509 - Failure in digest computation" );
        case MBEDTLS_X509_EMC_FAILED_TO_CONNECT_EMC_CORE:
            return( "X509 - Connection to Emercoin Core failed" );
        case MBEDTLS_X509_EMC_INCOMPATIBLE_ISSUER:
            return( "X509 - Received certificate doesn't support Emercoin NVS verification" );
        case MBEDTLS_X509_EMC_DIGEST_NOT_MATCHING:
            return( "X509 - Non-matching digest" );
        case MBEDTLS_X509_EMC_NO_NAME_IN_EMC_CORE:
            return( "X509 - No name found in Emercoin Core" );
        case MBEDTLS_X509_EMC_FAILED_TO_PARSE_JSON:
            return( "X509 - Failed to parse JSON request/response" );
        case MBEDTLS_X509_EMC_INVALID_HASH_FUNC:
            return( "X509 - Invalid hash function" );
        case MBEDTLS_X509_EMC_TOP_LEVEL_ELEMENT_IS_NOT_OBJECT:
            return( "X509 - No root object found in JSON response" );
    }
